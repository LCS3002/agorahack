...
// and bare numbers (e.g. "13" in "article 13" matches invoice references, postal codes, etc.)
const LOBBY_STOPWORDS = new Set([
  'article', 'the', 'and', 'for', 'with', 'that', 'this', 'from', 'into',
  'about', 'what', 'who', 'how', 'why', 'when', 'did', 'does', 'was', 'were',
  'tell', 'show', 'give', 'explain', 'new', 'old', 'all', 'any', 'its',
]);
...
export function extractSearchTerms(query: string, entities: string[]): string[] {
  ...
  const parts = raw
    ...
    // Drop pure numbers (e.g. "13") and question-word preamble
