      ...
      <section style={{
        ...
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: 64 }}>
            <h2 style={{ fontSize: 'clamp(28px, 4vw, 48px)', fontWeight: 200, letterSpacing: '-0.5px', color: '#1A1A18', margin: 0 }}>
              The team
            </h2>
            <span style={{ fontSize: 10, fontWeight: 500, color: 'rgba(26,26,24,0.3)', letterSpacing: '3px', textTransform: 'uppercase' }}>
              AgoraHacks 2026
            </span>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1px', background: 'rgba(26,26,24,0.08)' }}>
            {([
              ...
              <div key={p.name} style={{ background: '#F0EDE8', padding: '40px 40px 36px' }}>
                <div style={{ fontSize: 22, fontWeight: 300, color: '#1A1A18', marginBottom: 12, letterSpacing: '-0.2px' }}>{p.name}</div>
                <div style={{ fontSize: 13, fontWeight: 300, color: 'rgba(26,26,24,0.5)', lineHeight: 1.6, marginBottom: 32, whiteSpace: 'pre-line' }}>
                  {p.study}{'\n'}{p.at}
                </div>
                <div style={{ display: 'flex', gap: 16 }}>
                  <a href={p.linkedin} target="_blank" rel="noopener noreferrer" title="LinkedIn"
                    style={{ display: 'flex', alignItems: 'center', gap: 6, color: 'rgba(26,26,24,0.35)', fontSize: 11, fontWeight: 400, letterSpacing: '0.06em', textDecoration: 'none', transition: 'color 0.15s ease' }}
